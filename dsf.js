...
class HillClimbing {
  ...
  addEdge(from, to, cost = 1) {
      ...
      this.nodes[from].push({ node: to, cost: cost });
  }

  setHeuristic(node, value) {
      this.heuristic[node] = value;
  }
  ...
  search(start, goal) {
      ...
      while (queue.length > 0) {
          ...
          if (current.node === goal) {
              return `Goal ${goal} found with cost ${current.cost}`;
          }

          explored.add(current.node);

          let neighbors = this.nodes[current.node];
          if (neighbors) {
              neighbors.forEach(n => {
                  if (!explored.has(n.node)) {
                      let cost = current.cost + n.cost;
                      let nextNode = { node: n.node, cost: cost };
                      if (!queue.some(e => e.node === n.node && e.cost <= cost)) {
                          queue.push(nextNode);
                      }
                  }
              });
          ...
          queue = queue.sort((a, b) => (this.heuristic[a.node] + a.cost) - (this.heuristic[b.node] + b.cost));
      }
